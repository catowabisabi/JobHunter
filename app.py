...
def submit_job():
    ...
    if not personal_info:
        return jsonify({'status': 'error', 'message': 'Personal info not found.'}), 404

    # Build a complete and clean CV dictionary for the prompt
    cv_data = {
        "personal_info": personal_info.to_dict(),
        "work_experience": [exp.to_dict() for exp in experiences],
        "education": [edu.to_dict() for edu in education]
    }

    # Prepare prompt for Gemini
    prompt = f"""
    You are a professional resume and cover letter writing assistant.
    Based on the following job description and my CV data, please perform the following tasks and return the response as a single, valid JSON object.

    **Job Description:**
    {data['job_description']}
    
    **My CV Information (in JSON format):**
    {json.dumps(cv_data, ensure_ascii=False, indent=2, default=json_serial)}

    **Your Tasks:**

    1.  **Generate "cv_md":** A targeted CV in Markdown format.
        *   **Strict Formatting:** The Markdown structure MUST strictly follow this format:
            - `# Name`
            - `📍 Location | 📞 Phone | 📧 Email` (as a single line)
            - `🌐 Portfolio | Behance | GitHub` (as a single line)
            - `---` (horizontal rule)
            - `## Section Title` (e.g., Professional Summary, Core Competencies)
            - `### Job Title` for each role in experience sections.
        *   **Experience Filtering:** Analyze the Job Description. List all highly relevant work experiences under `## Professional Experience`. List all other, less relevant experiences under a new section `## Other Experience`.
        *   **Skill Filtering:** Only include skills from my CV Information that are relevant to the Job Description. Do not include a skill if it's completely irrelevant.
        *   **Dynamic Links:**
            - If the job appears to be IT/software/tech-related, include my GitHub link.
            - If the job appears to be design-related (Industrial, UI, UX), include my mono12.com and Behance links.
            - If the job is a general or other type of role, do not include any of these three links.
        *   **ATS Keywords:** Identify key skills and requirements (ATS keywords) from the Job Description. Naturally weave these keywords into the "Professional Summary" and "Professional Experience" bullet points where my experience supports them.

    2.  **Generate "cover_letter_en_md":** A professional cover letter in English, using a well-structured and aesthetically pleasing Markdown format.

    3.  **Generate "cover_letter_zh_md":** A professional cover letter in **Traditional Chinese (繁體中文)**. You MUST use Traditional Chinese characters. For example, use '軟體' and '設計', not '软件' and '设计'. The output MUST be in 繁體中文.

    4.  **Generate "job_details":** A JSON object containing key details extracted from the job description, with keys for "company_name", "position", "location", and "key_requirements".

    **Final Output Requirement:** Your entire output must be a single, valid JSON object with the keys "cv_md", "cover_letter_en_md", "cover_letter_zh_md", and "job_details". Do not include any text or formatting outside of this JSON structure.
    """
    
    try:
        # Call Gemini API
        logging.info(f"Calling Gemini API with model {GEMINI_MODEL}...")
        model = genai.GenerativeModel(GEMINI_MODEL)
        response = model.generate_content(prompt)
        
        # Log the raw response for debugging
        logging.info(f"Raw Gemini response text: {response.text}")

        # Clean the response text before parsing
        cleaned_text = response.text.strip().replace('```json', '').replace('```', '')
        
        # Fix invalid JSON escape sequences generated by the AI
        cleaned_text = re.sub(r'\\([^"\\/bfnrtu])', r'\\\\\1', cleaned_text)

        result = json.loads(cleaned_text)
        
        logging.info("Successfully parsed response from Gemini API.")
